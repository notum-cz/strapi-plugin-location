...
import { LocationQueryCombined, LogicalQuery } from 'src/types';

const createFilterMiddleware = (strapi: Core.Strapi) => {
  const db = strapi.db.connection;

  const modelsWithLocation = strapi.service(locationServiceUid).getModelsWithLocation();
  ...
  return async (ctx, next) => {
    ...
    const modelCondition = (model) => model.collectionName === _.snakeCase(collectionType);
    const collectionModel = !isComponentQuery
      ? Object.values(modelsWithLocation).find((model) => modelCondition(model))
    ...
    const componentModel =
      ...
      modelsWithLocation.find(
        (modelWithLocation) => modelWithLocation.uid === componentAttrField.component
      );
